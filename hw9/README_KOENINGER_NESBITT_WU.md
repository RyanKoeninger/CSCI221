...
- **WHAT** With the predicate class in the header file, it uses the operator() function to determine if a number is even or not. Then, depending on the outcome, it uses STL functions begin() and end() within replace_if() in the .cpp file to carry out the algorithm.

### 16. zero_evens_after_three()
- **WHERE** manager.cpp file and manager.hpp file. 
- **WHAT** With the predicate class in the header file, it uses the operator() function to determine if a number is even or not and if the algorithm has already gone through three even numbers. Then, depending on the outcome, it uses STL functions begin() and end() within replace_if() in the .cpp file to carry out the algorithm.